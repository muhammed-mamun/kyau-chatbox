...
export async function signup(req,res){
   ...
   try {
    ...
    const token=JWT.sign(
        {email:email,id:newUser._id},process.env.JWT_SECRET_KEY,{expiresIn:"7d"});
...
export async function login(req,res){
    try {
        ...
        if(!isPasswordCorrect){
            ...
            const token=JWT.sign({
                email:email,id:user._id},process.env.JWT_SECRET_KEY,{expiresIn:"7d"});
